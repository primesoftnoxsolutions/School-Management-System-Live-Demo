export default function ModuleDataPage({ title, dark = false }) {
  return (
    <section className="space-y-4">
      <div>
        <h2 className={`premium-title text-xl font-semibold ${dark ? "!text-white !shadow-none" : ""}`}>{title}</h2>
        <p className={`text-sm ${dark ? "text-[#9e9e9e]" : "text-sky-800/70"}`}>
