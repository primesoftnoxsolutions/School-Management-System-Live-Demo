...
import { isTeacherSubpage } from "../../constants/teacherNav";
...
function SchoolCrest({ className = "h-12 w-12" }) {
  return (
...
export default function Sidebar({ selected, onSelect, onLogout, role, user = null, dark = true, entering = false }) {
  ...
  const teacherParentActive = selected === "Teachers" || isTeacherSubpageActive;
  const feeParentActive = selected === "Fee Management" || isFeeSubpageActive;
  const studentParentActive = selected === "Students" || isStudentSubpageActive;

  // Auto-open only when landing on a subpage (don't force-open on parent, so arrow can collapse)
  useEffect(() => {
  ...
  const teacherSubpages = [
    { id: "Assigned Classes & Sections", label: "Assigned Classes", icon: teacherNavIconMap["Assigned Classes & Sections"] },
    { id: "Attendance", label: "Attendance", icon: teacherNavIconMap.Attendance },
    { id: "Syllabus", label: "Syllabus", icon: teacherNavIconMap.Syllabus },
    { id: "Duties", label: "Duties", icon: teacherNavIconMap.Duties },
    { id: "Time Table", label: "Time Table", icon: teacherNavIconMap["Time Table"] },
  ];

  const managementPlain = [
    { id: "Students Portfolios", label: "Students Portfolios", icon: navIconMap["Students Portfolios"] },
    { id: "Leaving & Character", label: "Leaving & Character", icon: navIconMap["Leaving & Character"] },
    { id: "Furniture & Assets", label: "Furniture & Assets", icon: navIconMap["Furniture & Assets"] },
    { id: "Payroll", label: "Payroll", icon: navIconMap.Payroll },
    { id: "Reports", label: "Reports", icon: navIconMap.Reports },
  ];
  ...
  const NavItem = ({ active, onClick, icon: Icon, label, compact }) => (
    <button
      type="button"
      onClick={onClick}
  ...
  return (
    <>
      <aside
        className={`fixed inset-y-0 left-0 z-30 hidden w-72 flex-col overflow-hidden lg:flex ${
          dark
      ...
      >
        ...
        <div className={`relative z-10 shrink-0 border-b px-5 pb-5 pt-6 ${dark ? "border-white/[0.06]" : "border-[#cfe3d5]/80"}`}>
          <div className="flex items-center gap-3.5">
            <SchoolCrest className="h-[52px] w-[46px] shrink-0" />
            <div className="min-w-0">
              <p className={`text-[11px] font-semibold uppercase tracking-[0.12em] ${dark ? "text-slate-400" : "text-[#6b7280]"}`}>
                Insaf Grammar
              </p>
              <h2 className={`mt-0.5 text-[18px] font-bold uppercase leading-none tracking-wide ${dark ? "text-emerald-400" : "text-[#1B5E3B]"}`}>
                High School
              </h2>
              <p className={`mt-1.5 text-[12px] ${dark ? "text-slate-500" : "text-[#9ca3af]"}`}>Management System</p>
            </div>
          </div>
        </div>
        ...
        >
          <NavItem
            ...
            icon={navIconMap.Dashboard}
            label="Dashboard"
          />

          <ExpandableNav
            {...expandProps}
            active={teacherParentActive}
            label="Teacher Page"
            icon={teacherNavIconMap["Teacher Page"] || navIconMap.Teachers}
            menuOpen={teacherMenuOpen}
            onNavigate={() => {
              onSelect("Teachers");
              setTeacherMenuOpen(true);
            }}
            onToggle={() => setTeacherMenuOpen((open) => !open)}
            ariaExpand="Expand teacher pages"
            ariaCollapse="Collapse teacher pages"
          >
          ...
          <ExpandableNav
            {...expandProps}
            active={studentParentActive}
            label="Students"
            icon={navIconMap.Students}
            menuOpen={studentMenuOpen}
            onNavigate={() => {
              onSelect("Students");
              setStudentMenuOpen(true);
            }}
            onToggle={() => setStudentMenuOpen((open) => !open)}
            ariaExpand="Expand student pages"
            ariaCollapse="Collapse student pages"
          >
            <StudentNavSubmenu
          ...
          <ExpandableNav
            ...
            icon={navIconMap["Fee Management"]}
            menuOpen={feeMenuOpen}
            onNavigate={() => {
              onSelect("Fee Management");
              setFeeMenuOpen(true);
            }}
          ...
          <div className="relative z-20 px-4 pb-2 pt-2">
            <div
              className={`overflow-hidden rounded-xl border shadow-sm ${
                dark ? "border-white/[0.08] bg-white/[0.04]" : "border-[#d7e5db] bg-white"
              }`}
            >
              <div className={`border-b px-3 py-2 ${dark ? "border-white/[0.06]" : "border-[#e8f0eb]"}`}>
                <p className={`text-[10px] font-semibold uppercase tracking-[0.08em] leading-none ${dark ? "text-emerald-300/80" : "text-[#1B5E3B]"}`}>
                  Login as
                </p>
                <p className={`mt-1 text-[13.5px] font-semibold leading-tight ${dark ? "text-white" : "text-[#1a2e22]"}`}>
                  {role === "SUPER_ADMIN" ? "Super Admin" : user?.fullName || user?.role || "Admin"}
                </p>
                <p className={`mt-0.5 truncate text-[11px] leading-tight ${dark ? "text-slate-400" : "text-[#6b7280]"}`}>
              ...
              <button
                ...
                className={`flex w-full items-center gap-2.5 px-3 py-2 text-left text-[13px] font-medium outline-none transition-colors focus:outline-none ${
                  dark
                    ? "text-rose-300 hover:bg-rose-500/10"
                    : "text-[#4a5568] hover:bg-[#fff8f8]"
                }`}
              >
                <IconLogout className={`h-4 w-4 ${dark ? "text-rose-400" : "text-[#e53935]"}`} />
                <span>Log Out</span>
              </button>
            </div>
          </div>
