...
export function DropdownBackdrop({ onClose }) {
  return createPortal(
    <div
      className="super-admin-overlay-backdrop fixed inset-0 z-[920] bg-slate-900/35 backdrop-blur-[2px]"
      aria-hidden="true"
      onMouseDown={onClose}
    />,
    document.body
...
export function DropdownMenuPortal({ menuRef, style, className = "", dark = false, children }) {
  return createPortal(
    <div
      ref={menuRef}
      className={`fixed z-[921] overflow-hidden rounded-xl shadow-xl ${
        dark
